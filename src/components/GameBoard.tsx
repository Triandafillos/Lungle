...
export default function GameBoard({ progress, makeGuess }: Props) {
        ...
        <div className="flex flex-col gap-3 flex-1 w-7/10">
            <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                ...
                {progress.guesses.map((guess, index) => (
                    <WordGuess key={index} length={5} text={guess} progress={progress} />
                ))}
                ...
                {Array.from({
                    ...
                    <WordGuess key={index} disabled={index > 0} length={5} progress={progress} />
                ))}
